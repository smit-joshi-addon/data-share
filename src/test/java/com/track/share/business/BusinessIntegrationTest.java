...
class BusinessIntegrationTest {
    ...
    void setUp() {
        ...
        restTemplate.exchange(API_AUTH_PATH + "/register", HttpMethod.POST, new HttpEntity<>(registerUser),
                Void.class);
        // when
    ...
    void shouldReturnUnauthorizedWhenAuthenticationTokenIsInvalidOrExpired() {
        ...
        // PUT
        invalidTokenResponse = restTemplate.exchange(API_BUSINESS_PATH + "/1", HttpMethod.PUT, new HttpEntity<>(headers),
                String.class);
        // then for PUT
        assertThat(invalidTokenResponse.getStatusCode()).isEqualTo(HttpStatus.UNAUTHORIZED);

        // DELETE
        invalidTokenResponse = restTemplate.exchange(API_BUSINESS_PATH + "/1", HttpMethod.DELETE, new HttpEntity<>(headers),
                String.class);
        // then for DELETE
        assertThat(invalidTokenResponse.getStatusCode()).isEqualTo(HttpStatus.UNAUTHORIZED);
    }
